export default class FormValidator {
  constructor(values, form) {
    this._values = values;
    this._form = form;
    this._formInputElements = this._form.querySelectorAll(
      this._values.inputSelector
    );
  }
  ...
  _showInputError(inputElement, errorMessage) {
    const errorMessageContainer = this._form.querySelector(
  ...
  _toggleSubmitButton() {
    ...
    if (
      Array.from(this._form.elements).some((element) =>
        this._isInvalid(element)
      )
    ) {
